...
class _ScoreScreenState extends State<ScoreScreen> {
            ...
            Expanded(
              child: Container(
                ...
                child: Consumer<ScoreProvider>(
                  builder: (BuildContext context, ScoreProvider value,
                      Widget? child) {
  ...
  deleteScore(ScoreProvider value, int index) {
    value.deleteScore(value.arrScore[index].id ?? 0);
    value.arrScore.removeAt(index);
  }

  Color? conditionalolor(ScoreProvider value, int index) {
    if ((value.arrScore[index].score ?? 0) <= 10) {
      return Colors.red;
    }
    if ((value.arrScore[index].score ?? 0) >= 70) {
      return Colors.green;
    }
  }
}
